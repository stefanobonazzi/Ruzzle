...
public class Ricerca {
	...
	public List<Pos> trovaParola(String parola, Board board) {
		for(Pos p: board.getPositions()) {
			//Verifico se la lettera in pos è == alla prima lettera della parola
			if(board.getCellValueProperty(p).get().charAt(0) == parola.charAt(0)) {
				//Se la ricerca nel board è uguale alla prima lettera di parola posso far partire la ricorsione
